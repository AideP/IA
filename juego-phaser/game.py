...
directory_to_save_datasets = 'C:/Users/perez/PycharmProjects/datasets'
directory_to_save_desition_tree = 'C:/Users/perez/PycharmProjects/desition_tree'
...
directory_to_save_neural_network = 'C:/Users/perez/PycharmProjects/neural_network'
...
knn_model = None
directory_to_save_knn = 'C:/Users/perez/PycharmProjects/knn_model'
...
pygame.display.set_caption("Juego: Phaser")
...
def cargar_modelo_neural_network():
    ...
    try:
        model_path_horizontal_ball = os.path.join(directory_to_save_neural_network,
                                                  'neural_network_model_horizontal_ball.keras')
        neural_network_trained_horizontal_ball = load_model(model_path_horizontal_ball)
        model_path_vertical_ball = os.path.join(directory_to_save_neural_network,
                                                'neural_network_model_vertical_ball.keras')
...
def generate_neural_network():
    ...
    X_train_horizontal, X_test_horizontal, y_train_horizontal, y_test_horizontal = train_test_split(X_horizontal,
                                                                                                    y_horizontal,
                                                                                                    test_size=0.2,
                                                                                                    random_state=42)
    X_train_vertical, X_test_vertical, y_train_vertical, y_test_vertical = train_test_split(X_vertical, y_vertical,
                                                                                            test_size=0.2,
                                                                                            random_state=42)
    ...
    # Entrenar el modelo
    model_horizontal_ball.fit(X_train_horizontal, y_train_horizontal, epochs=50, batch_size=32, validation_split=0.2,
                              verbose=1)
    model_vertical_ball.fit(X_train_vertical, y_train_vertical, epochs=50, batch_size=32, validation_split=0.2,
                            verbose=1)

    # Evaluar el modelo
    loss_horizontal, accuracy_horizontal = model_horizontal_ball.evaluate(X_test_horizontal, y_test_horizontal,
                                                                          verbose=0)
...
def cargar_modelo_decision_tree():
    ...
    try:
        decision_tree_trained_horizontal_ball = joblib.load(
            directory_to_save_desition_tree + '/decision_tree_model_horizontal_ball.joblib')
        decision_tree_trained_vertical_ball = joblib.load(
            directory_to_save_desition_tree + '/decision_tree_model_vertical_ball.joblib')
...
def generate_desition_treee():
    ...
    cleaned_csv_path_horizontal_ball = os.path.join(directory_to_save_desition_tree,
                                                    'dataset_cleaned_horizontal_ball.csv')
    ...
    model_path_horizontal_ball = os.path.join(directory_to_save_desition_tree,
                                              'decision_tree_model_horizontal_ball.joblib')
...
def print_menu_options():
    lineas = [
        "[M] Manual [N] RW [D] DT [K] KNN [S] Guardar [T] Entrenar [Q] Salir "
    ]
                ...
                if prediction_counter_horizontal_ball % 1 == 0 and prediction_counter_vertical_ball % 1 == 0:
                    if mode_neural_network and neural_network_trained_horizontal_ball is not None:
                        ...
                        if predecir_salto_neural_network(velocidad_bala, desplazamiento_bala) and en_suelo:
                            salto = True
                            en_suelo = False
                        if predecir_retroceso_neural_network(velocidad_bala2,
                                                             bala2.y - jugador.y) and en_pocision_inicial:
                            retroceso = True
                            en_pocision_inicial = False
                    if salto:
                        manejar_salto()
